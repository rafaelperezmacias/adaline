...
//    }
    ...
    public void run() {
        ...
        while ( !done && epoch < epochs ) {
            ...
            for ( int i = 0; i < X.length; i++ ) {
                ...
                if ( error != 0.0 ) {
                    ...
                    window.updateWeightsForPerceptron();
                    try {
                        Thread.sleep(500);
            ...
            epoch++;
            window.updateEpochForPerceptron(epoch, done, false);
            try {
                Thread.sleep(100);
            } catch (InterruptedException e) {
                throw new RuntimeException(e);
        ...
        window.updateEpochForPerceptron(epoch, done, true);
        window.updateWeightsForPerceptron();
        window.setModelForPerceptron(new Model(dataSet, epochs, epoch, weights, done));
    }
